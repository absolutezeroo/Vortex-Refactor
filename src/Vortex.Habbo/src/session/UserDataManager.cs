...
public class UserDataManager : IUserDataManager
{
    ...
    public IUserData? GetUserData(int userId) => GetUserDataByType(userId, TYPE_USER);
    ...
    {
        _byRoomIndex.TryGetValue(roomObjectId, out var data);
        return data;
    }
    ...
    public IReadOnlyList<string>? GetUserBadges(int userId)
    {
        ...
        _badges.TryGetValue(userId, out var badges);
        return badges ?? [];
    }
    ...
    public IUserData? GetPetUserData(int petId) => GetUserDataByType(petId, TYPE_PET);

    /// @see UserDataManager.as::getRentableBotUserData
    public IUserData? GetRentableBotUserData(int botId) => GetUserDataByType(botId, 4);

    /// @see UserDataManager.as::requestPetInfo
    public void RequestPetInfo(int petId)
    {
        var data = GetPetUserData(petId);
