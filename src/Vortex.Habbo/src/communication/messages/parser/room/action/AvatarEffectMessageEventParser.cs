using Vortex.Core.Communication.Messages;

namespace Vortex.Habbo.Communication.Messages.Parser.Room.Action;

/// @see com.sulake.habbo.communication.messages.parser.room.action.AvatarEffectMessageEventParser
public class AvatarEffectMessageEventParser : IMessageParser
{
    public int UserId { get; private set; }
    public int EffectId { get; private set; }
    public int DelayMilliSeconds { get; private set; }

    public bool Flush() { UserId = 0; return true; }

    public bool Parse(IMessageDataWrapper param1)
    {
        if (param1 == null)
        {
            return false;
        }
        UserId = param1.ReadInteger();
        EffectId = param1.ReadInteger();
        DelayMilliSeconds = param1.ReadInteger();
        return true;
    }
}
